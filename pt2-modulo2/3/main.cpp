...
ImageClass* GrayScale(ImageClass *imageIn)
{
	// Tarefa 2:
    ...
    for(x=0; x<imageIn->SizeX(); x++)
    {
        for(y=0; y<imageIn->SizeY(); y++)
        {
			...
			imageOut->DrawPixel(x, y, gs, gs, gs);
        }
    }
    return imageOut;
}
...
void GrayScale()
{
    ...
    ImageClass *tmp = GrayScale(Imagem1);
    tmp->CopyTo(Imagem2);
    /**/
}
...
ImageClass EqualizaHistograma()
{
    ...
    ImageClass *imageIn  = new ImageClass(sizeX, sizeY);
    ImageClass *imageOut = new ImageClass(sizeX, sizeY);

	for( int x=0; x<sizeX; x++ )
	{
		for( int y=0; y<sizeY; y++ )
		{
			double _y = Y[x][y] + br_level;
			double _i = I[x][y];
			double _q = Q[x][y];
...
int main ( int argc, char** argv )
{
	...
	cout << " 4 - equalizar histograma (a partir de YIQ)" << endl;
	cout << " 5 - zoom in quadriculado" << endl;
	cout << " 6 - zoom in linear" << endl;
	cout << " 7 - zoom out" << endl;
	cout << " 8 - zoom in com interpolacao por splines quadraticos" << endl;
